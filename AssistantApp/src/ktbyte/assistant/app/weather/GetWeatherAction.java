...
public class GetWeatherAction extends Action {
    ...
    public void doCommand(String command) {
        ...
        if (words.contains("zip") || words.contains("zipcode")) {
            req = Unirest.get("https://api.openweathermap.org/data/2.5/weather").queryString("zip", location)
                    .queryString("appid", API_KEY).queryString("units", "imperial");
        } else {
            req = Unirest.get("https://api.openweathermap.org/data/2.5/weather").queryString("q", location)
                    .queryString("appid", API_KEY).queryString("units", "imperial");
        }
